...
public class AiNormalTester : MonoBehaviour
{
    private readonly List<Collider2D> others = new List<Collider2D>();
    private new Collider2D collider;
    private AiMovementBase aiMovement;
    [SerializeField]
    ...
    void Start ()
    {
        aiMovement = transform.parent.gameObject.GetComponent<AiMovementBase>();
    ...
    void Update ()
    {
        ...
        if (SurfaceCount == 0)
        {
            ...
            if(aiMovement != null) aiMovement.Normal = Vector3.zero;
            return;
        }
        ...
        if(aiMovement != null)
            aiMovement.Normal = Quaternion.Euler(0, 0, GetAngle(avg, dirtyShortestAngle) + 90) * Vector3.right; // FIXME
